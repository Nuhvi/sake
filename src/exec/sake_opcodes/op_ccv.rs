use bitcoin::{ScriptBuf, hashes::sha256, secp256k1};
...
impl<'a> Exec<'a> {
    fn handle_op_ccv(&mut self) -> Result<(), ExecError> {
        ...
        let pk_raw = self.stack.pop().unwrap().as_bytes();
        ...
        let naked_key = if pk_raw.is_empty() {
            ...
            secp256k1::XOnlyPublicKey::from_slice(&pk_raw).unwrap()
            // .map_err(|_| ExecError::PubKeyType)?
        } else {
            todo!();
            // return Err(ExecError::PubKeyType);
        };
        ...
        //
        // // 6. Verify Script
        // let expected_script = ScriptBuf::new_p2tr_tweaked(final_key);
        ...
        match mode {
            // TODO: support other modes than ignore
            // 0 => {
